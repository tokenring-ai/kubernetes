...
import { KubeConfig, CoreV1Api, DiscoveryApi, CustomObjectsApi } from '@kubernetes/client-node';
...
export default class KubernetesService extends Service {
 ...
 constructor({ clusterName, apiServerUrl, namespace, token, clientCertificate, clientKey, caCertificate }) {
  super();
  if (!clusterName) {
 ...
 async listAllApiResourceTypes(registry) {
      ...
      const userConfig = { name: 'service-user' };
      if (this.token) {
      ...
      const processApiGroupVersion = async (groupVersion, groupNameForLog) => {
        try {
          ...
          for (const resource of apiResourceList.resources) {
            ...
            const group = resource.group || groupVersion.split('/')[0] || ''; // Prefer explicit group, fallback for core v1
            const version = resource.version || groupVersion.split('/')[1]; // Prefer explicit version
              ...
              for (const ns of namespacesToScan) {
                try {
                  ...
                  if (result && result.items) {
                    result.items.forEach(item => allResources.push({
                      group: group,
                      version: version,
                      kind: kind,
